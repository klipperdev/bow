...
<template>
    <v-container fluid>
        <k-data-list ref="dataList" :headers="headers" :fetch-request="fetchRequest" metadata="role">
            ...
            <template v-slot:data-table.item.label="{item}">
                <router-link class="text--primary" :to="{name: 'settings-org-role', params: {org: $org, id: item.id}}">
                    {{ $oc(item).label() }}
                </router-link>
            </template>
